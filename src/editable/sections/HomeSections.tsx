...
function ListingCard({ post, href }: { post: SitePost; href: string }) {
  return (
    <Link href={href} className="group grid min-h-[210px] gap-4 rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-xl sm:grid-cols-[96px_minmax(0,1fr)]">
      ...
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <span className="rounded-full bg-[var(--slot4-accent-soft)] px-3 py-1 text-[11px] font-black uppercase tracking-[0.12em] text-[var(--slot4-accent)]">Business</span>
          <span className="inline-flex items-center gap-1 text-xs font-bold text-[var(--slot4-muted-text)]"><MapPin className="h-3.5 w-3.5" /> {getLocation(post)}</span>
        </div>
        <h3 className="mt-3 line-clamp-2 text-xl font-black leading-tight tracking-[-0.03em]">{post.title}</h3>
        <p className="mt-2 line-clamp-2 text-sm leading-6 text-[var(--slot4-muted-text)]">{getExcerpt(post, 125)}</p>
        <span className="mt-4 inline-flex items-center gap-2 text-sm font-black text-[var(--slot4-accent)]">View details <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></span>
      </div>
    </Link>
  )
}
...
function ReviewCard({ post, index }: { post: SitePost; index: number }) {
  return (
    <article className="w-[280px] shrink-0 snap-start rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-5 shadow-sm">
      <div className="flex items-center gap-3">
        <div className="grid h-11 w-11 place-items-center rounded-full bg-gradient-to-br from-[#ff7a1a] to-[var(--slot4-accent)] text-sm font-black text-white">{post.title.slice(0, 1)}</div>
        <div className="min-w-0">
          <p className="truncate text-sm font-black">{['Jordan M.', 'Priya S.', 'Carlos R.', 'Mina K.'][index % 4]}</p>
          <div className="mt-1 flex text-amber-400">{Array.from({ length: 5 }).map((_, star) => <Star key={star} className="h-4 w-4 fill-current" />)}</div>
        </div>
      </div>
      <p className="mt-4 line-clamp-4 text-sm leading-6 text-[var(--slot4-muted-text)]">{getExcerpt(post, 150) || 'The listing had the address, phone number, and business details I needed before visiting.'}</p>
      <div className="mt-5 border-t border-[var(--editable-border)] pt-4 text-center text-sm font-black">{post.title}</div>
    </article>
  )
}
...
export function EditableHomeHero({}: HomeSectionProps) {
  return (
    <section className="bg-[var(--slot4-page-bg)] px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[var(--editable-container)] overflow-hidden rounded-2xl bg-[var(--slot4-dark-bg)] shadow-[0_24px_90px_rgba(15,23,42,0.14)]">
        <div className="relative min-h-[470px] bg-[url('/Hero-background.jpg?height=900&width=1600')] bg-cover bg-center">
          ...
          <div className="relative z-10 flex min-h-[470px] flex-col items-center justify-center px-5 py-12 text-center text-white">
            ...
            <form action="/search" className="mt-8 grid w-full max-w-3xl overflow-hidden rounded-full bg-[var(--slot4-surface-bg)] text-left shadow-2xl sm:grid-cols-[1fr_0.85fr_auto]">
              <label className="flex items-center gap-3 border-b border-[var(--editable-border)] px-5 py-4 sm:border-b-0 sm:border-r">
              ...
              <label className="flex items-center gap-3 px-5 py-4 sm:border-r border-[var(--editable-border)]">
                <MapPin className="h-5 w-5 text-[var(--slot4-muted-text)]" />
                <input name="category" placeholder="City or category" className="min-w-0 flex-1 bg-transparent text-sm font-bold text-[var(--slot4-page-text)] outline-none placeholder:text-[var(--slot4-muted-text)]" />
              </label>
              <button className="m-2 inline-flex items-center justify-center rounded-full bg-amber-400 px-5 py-3 text-sm font-black text-[var(--slot4-page-text)]" type="submit" aria-label="Search businesses"><ArrowRight className="h-5 w-5" /></button>
            </form>
...
export function EditableStoryRail({ primaryTask, primaryRoute, posts }: HomeSectionProps) {
  ...
  return (
    <section className="bg-[var(--slot4-page-bg)] px-4 py-12 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[var(--editable-container)]">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--slot4-accent)]">Featured local businesses</p>
            <h2 className="mt-2 text-3xl font-black tracking-[-0.04em]">Recently listed businesses</h2>
          </div>
          <Link href={primaryRoute} className="rounded-full bg-[var(--slot4-accent)] px-5 py-3 text-sm font-black text-white">View all</Link>
        </div>
        {items.length ? (
          ...
          <div className="mt-7 rounded-2xl border border-dashed border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-10 text-center">
            <Building2 className="mx-auto h-10 w-10 text-[var(--slot4-muted-text)]" />
...
export function EditableMagazineSplit({ posts }: HomeSectionProps) {
  ...
  return (
    <section className="bg-[var(--slot4-lavender)] px-4 py-12 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[var(--editable-container)]">
        <h2 className="text-center text-3xl font-black tracking-[-0.04em]">Latest reviews</h2>
...
export function EditableTimeCollections({ primaryRoute }: HomeSectionProps) {
  return (
    <section className="bg-[var(--slot4-surface-bg)] px-4 py-12 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[var(--editable-container)]">
        <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--slot4-accent)]">Browse by city</p>
            <h2 className="mt-2 text-3xl font-black tracking-[-0.04em]">Popular places in the directory</h2>
            <p className="mt-4 text-sm leading-7 text-[var(--slot4-muted-text)]">Find restaurants, home services, medical offices, shops, and professional services by location.</p>
            <Link href={primaryRoute} className="mt-6 inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-black">Open business listings <ArrowRight className="h-4 w-4" /></Link>
          </div>
          <div className="grid gap-x-8 gap-y-4 sm:grid-cols-2 lg:grid-cols-3">
            {cities.map((city) => (
              <Link key={city} href={`/search?category=${encodeURIComponent(city)}`} className="text-sm font-black uppercase text-[var(--slot4-accent)] hover:underline">{city}</Link>
            ))}
          </div>
        </div>
        <div className="mt-12 grid gap-5 md:grid-cols-3">
          {[
            ...
            <div key={item.title} className="rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-cream)] p-6">
...
export function EditableHomeCta() {
  return (
    <section id="get-app" className="bg-[var(--slot4-page-bg)] px-4 py-14 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[var(--editable-container)] rounded-2xl bg-[linear-gradient(110deg,#ffb44a,#ff3838_45%,#8e2db5)] px-6 py-12 text-center text-white shadow-[0_24px_90px_rgba(142,45,181,0.18)] sm:px-10">
