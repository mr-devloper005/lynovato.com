...
import { ArrowLeft, Bookmark, Building2, Camera, CheckCircle2, Download, ExternalLink, FileText, Globe2, Mail, MapPin, MessageCircle, Navigation, Phone, Tag, UserRound } from 'lucide-react'
...
function BackLink({ task }: { task: TaskKey }) {
  ...
  return (
    <Link href={taskConfig?.route || '/'} className="inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)]/70 px-4 py-2 text-sm font-black">
...
function ArticleDetail({ post, related, comments }: { post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_350px] lg:px-8 lg:py-16">
      <article className="min-w-0 rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-5 shadow-[0_30px_90px_rgba(15,23,42,0.09)] sm:p-8 lg:p-12">
        ...
        {images[0] ? <img src={images[0]} alt="" className="mt-8 max-h-[620px] w-full rounded-2xl object-cover" /> : null}
...
function ListingDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto max-w-[var(--editable-container)] px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
      ...
      <div className="rounded-2xl bg-[var(--slot4-lavender)] p-6 shadow-[0_24px_80px_rgba(15,23,42,0.08)] sm:p-8">
        <div className="flex flex-wrap justify-between gap-5">
          <div className="grid gap-5 sm:grid-cols-[118px_1fr]">
            <div className="flex h-28 w-28 items-center justify-center overflow-hidden rounded-2xl bg-[var(--slot4-panel-bg)] ring-1 ring-[var(--editable-border)]">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <p className="text-sm font-black text-[var(--slot4-accent)]">{categoryOf(post, 'Business')}</p>
                <span className="text-[var(--slot4-muted-text)]">&bull;</span>
                <p className="text-sm font-bold text-[var(--slot4-muted-text)]">{address || 'Local business'}</p>
              </div>
              <h1 className="mt-2 max-w-3xl text-3xl font-black leading-tight tracking-[-0.04em] sm:text-4xl">{post.title}</h1>
              <div className="mt-5 flex flex-wrap gap-3">
                {phone ? <a href={`tel:${phone}`} className="rounded-full bg-[var(--slot4-accent)] px-5 py-3 text-sm font-black text-white">Call business</a> : null}
                {website ? <a href={externalUrl(website)} target="_blank" rel="noreferrer" className="rounded-full border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-black">Visit website</a> : null}
                <Link href="/contact" className="rounded-full border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-black">Claim listing</Link>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mt-5 grid gap-6 lg:grid-cols-[minmax(0,1fr)_365px]">
        <article className="space-y-5">
          <section id="review" className="rounded-2xl bg-[var(--slot4-surface-bg)] p-6 shadow-sm">
            <div className="rounded-2xl bg-[var(--slot4-lavender)] p-7 text-center">
              <h2 className="text-xl font-black tracking-[-0.03em]"> {post.title}</h2>
            </div>
          </section>

          <section className="rounded-2xl bg-[var(--slot4-surface-bg)] p-6 shadow-sm">
          ...
          <section className="rounded-2xl bg-[var(--slot4-surface-bg)] p-6 shadow-sm">
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                {address ? <p className="mt-1 text-sm font-bold text-[var(--slot4-muted-text)]">{address}</p> : null}
              </div>
              {address ? <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address)}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] px-4 py-2 text-sm font-black">Get directions <Navigation className="h-4 w-4" /></a> : null}
            </div>
          </section>
...
function ClassifiedDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-7 px-4 py-10 sm:px-6 lg:grid-cols-[0.82fr_1.18fr] lg:px-8 lg:py-16">
      <aside className="rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-dark-bg)] p-7 text-[var(--slot4-dark-text)] shadow-xl lg:sticky lg:top-24 lg:self-start">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          {phone ? <a href={`tel:${phone}`} className="rounded-full bg-[var(--slot4-page-bg)] px-5 py-3 text-sm font-black text-[var(--slot4-page-text)]">Call now</a> : null}
          {email ? <a href={`mailto:${email}`} className="rounded-full border border-white/25 px-5 py-3 text-sm font-black">Email</a> : null}
        </div>
      </aside>
      <article className="rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] sm:p-9">
  ...
  return (
    <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[0.72fr_1.28fr]">
        <aside className="rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-7 lg:sticky lg:top-24 lg:self-start">
        ...
        <div className="columns-1 gap-5 space-y-5 md:columns-2">
          {(images.length ? images : ['/placeholder.svg?height=900&width=1200']).map((image, index) => (
            <figure key={`${image}-${index}`} className="break-inside-avoid overflow-hidden rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] shadow-sm">
...
function BookmarkDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-16">
      <article className="rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-7 shadow-[0_30px_90px_rgba(15,23,42,0.08)] sm:p-10">
        <BackLink task="sbm" />
        <div className="mt-10 flex h-20 w-20 items-center justify-center rounded-2xl bg-[var(--slot4-dark-bg)] text-[var(--slot4-dark-text)]"><Bookmark className="h-9 w-9" /></div>
    ...
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-16">
      <article className="rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] sm:p-9">
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-[120px_1fr]">
          <div className="flex h-28 w-28 items-center justify-center rounded-2xl bg-[var(--slot4-dark-bg)] text-[var(--slot4-dark-text)]"><FileText className="h-12 w-12" /></div>
          <div>
        ...
        <BodyContent post={post} />
        {fileUrl ? (
          <div className="mt-8 overflow-hidden rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-page-bg)]">
            <div className="flex items-center justify-between gap-3 border-b border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-4">
...
function ProfileDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[420px_minmax(0,1fr)] lg:px-8 lg:py-16">
      <aside className="rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-8 text-center shadow-[0_30px_90px_rgba(15,23,42,0.08)] lg:sticky lg:top-24 lg:self-start">
        <BackLink task="profile" />
        <div className="mx-auto mt-10 flex h-40 w-40 items-center justify-center overflow-hidden rounded-full bg-[var(--slot4-page-bg)] ring-1 ring-[var(--editable-border)]">
          {images[0] ? <img src={images[0]} alt="" className="h-full w-full object-cover" /> : <UserRound className="h-16 w-16 text-[var(--slot4-muted-text)]" />}
        </div>
      ...
      <article className="rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-7 shadow-sm sm:p-10">
        <BodyContent post={post} />
...
function InfoPanel({ address, phone, email, website }: { address?: string; phone?: string; email?: string; website?: string }) {
  ...
  return (
    <div className="rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-5 shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
      ...
      <div className="mt-4 divide-y divide-[var(--editable-border)]">
        {items.map((item, index) => (
          <div key={`${item.value}-${index}`} className="flex gap-3 py-4 text-sm font-bold text-[var(--slot4-muted-text)]">
            <item.icon className="mt-0.5 h-4 w-4 shrink-0" />
            {item.href ? <a href={item.href} target={item.href.startsWith('http') ? '_blank' : undefined} rel={item.href.startsWith('http') ? 'noreferrer' : undefined} className="break-all hover:text-[var(--slot4-accent)]">{item.value}</a> : <span>{item.value}</span>}
          </div>
        ))}
        <Link href="/contact" className="flex gap-3 py-4 text-sm font-bold text-[var(--slot4-muted-text)] underline"><WrenchIcon /> Improve this listing</Link>
      </div>
    </div>
  )
}
...
function MapBox({ src, label }: { src: string; label: string }) {
  return (
    <div className="mt-4 overflow-hidden rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)]">
...
function ContactAction({ website, phone, email }: { website?: string; phone?: string; email?: string }) {
  ...
  return (
    <div className="mt-5 rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-5 shadow-sm">
      <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--slot4-muted-text)]">Quick actions</p>
      <div className="mt-4 flex flex-wrap gap-3">
        {website ? <a href={externalUrl(website)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full bg-[var(--slot4-accent)] px-4 py-2 text-sm font-black text-white">Website <ExternalLink className="h-4 w-4" /></a> : null}
...
function RelatedPanel({ task, related, compact = false }: { task: TaskKey; post?: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)]/70 p-5 backdrop-blur">
          <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--slot4-muted-text)]">About this post</p>
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-3 transition hover:-translate-y-0.5 hover:shadow-lg">
      {image && task !== 'sbm' ? <img src={image} alt="" className="h-20 w-20 shrink-0 rounded-xl object-cover" /> : <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-xl bg-[var(--slot4-page-bg)]"><FileText className="h-6 w-6 text-[var(--slot4-muted-text)]" /></div>}
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-10 rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)]/70 p-5">
      <div className="flex items-center gap-2 text-lg font-black"><MessageCircle className="h-5 w-5" /> Comments</div>
      <div className="mt-5 grid gap-3">
        {comments.slice(0, 5).map((comment) => (
          <div key={comment.id} className="rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-4">
