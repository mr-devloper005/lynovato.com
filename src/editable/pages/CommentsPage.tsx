    ...
    <EditableSiteShell>
      <main className="bg-[var(--slot4-page-bg)] text-[var(--slot4-page-text)]">
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
          <section className="rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-6 shadow-sm sm:p-8">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <p className="inline-flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.24em] text-[var(--slot4-muted-text)]">
                  <MessageSquare className="h-4 w-4" /> Local comments
                </p>
                <h1 className="mt-4 text-4xl font-extrabold tracking-[-0.04em] sm:text-5xl">Comments</h1>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="relative w-full sm:max-w-md">
                ...
                <input
                  ...
                  className="h-11 w-full rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] pl-9 pr-3 text-sm outline-none"
                />
              </div>
          ...
          {visibleComments.length ? (
            <section className="mt-8 grid gap-4">
              {visibleComments.map((item) => (
                <article key={`${item.articleSlug}-${item.id}`} className="rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-5 shadow-sm">
            ...
            <section className="mt-8 rounded-2xl border border-dashed border-[var(--editable-border)] bg-[var(--slot4-surface-bg)]/70 p-8 text-center">
              <h2 className="text-xl font-extrabold">No comments yet</h2>
